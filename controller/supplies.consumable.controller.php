...
function getAllConsumableItems($conn){
   ...
   return $conn->query("SELECT * FROM items INNER JOIN suppliers ON items.supplier = suppliers.supplier_id INNER JOIN categories ON items.item_category = categories.cat_id INNER JOIN measurement ON items.measurement = measurement.id WHERE items.item_type = 1");
...
if(isset($_POST['itemCode']) && isset($_POST['itemDesc']) && isset($_POST['supplier']) && isset($_POST['categoryType']) && isset($_POST['measurementType']) && isset($_POST['itemQuantity'])){

     $query = $conn->prepare("INSERT INTO items (item_code,item_description,item_category,measurement,supplier,quantity,date_modified,available,item_type) VALUES(?,?,?,?,?,?,?,1,1)");
   ...
   $added =  $query->execute();
     $text = ''.$_POST['itemDesc'].' - Consumable Item has been Added';
...
if(isset($_POST['editItemId']) && isset($_POST['editDateModified']) && isset($_POST['editItemCode']) && isset($_POST['editItemDesc']) && isset($_POST['editCategoryType']) && isset($_POST['editSupplier']) && isset($_POST['editMeasurementType'])&& isset($_POST['editItemQuantity'])){
    ...
    $query = $conn->prepare("UPDATE items SET item_code= ? , item_description= ? , item_category= ? , measurement= ? ,supplier= ? , quantity= ? , date_modified= ? WHERE item_id = ?");
    $query->bind_param('ssiiiisi',$_POST['editItemCode'],$_POST['editItemDesc'],$_POST['editCategoryType'],$_POST['editMeasurementType'],$_POST['editSupplier'],$_POST['editItemQuantity'],$_POST['editDateModified'],$_POST['editItemId']);
