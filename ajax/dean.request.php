...
$requests = getAllDeanRequests($conn,$type);
...
<tbody>
    ...
    <tr>
        ...
        <td>
           ...
           <?php if($request['dean_status'] == 1 || $request['dean_status'] == 3){?>
                <button onclick="approveRequest(<?=$request['request_id']?>)" class="btn-sm btn-primary">Approve</button>
             <?php }?>

            <?php if($request['dean_status'] == 1 || $request['dean_status'] == 2){?>
                <button data-toggle="modal" data-target="#changeDeanStatus" onclick="setCurrent(<?=$request['request_id']?>)" class="btn-sm btn-danger">Disapprove</button>
