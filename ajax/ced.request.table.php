...
$requests = getAllCedRequests($conn,$type);
...
<tbody>
    ...
    <tr>
        ...
        <td>
              <?php if($request['ced_status'] == 1 || $request['ced_status'] == 3){?>
                 <button onclick="approveRequest(<?=$request['request_id']?>)" class="btn-sm btn-primary">Approve</button>
             <?php }?>

            <?php if($request['ced_status'] == 1 || $request['ced_status'] == 2){?>
                <button data-toggle="modal" data-target="#changeCedStatus" onclick="setCurrent(<?=$request['request_id']?>)" class="btn-sm btn-danger">Disapprove</button>
