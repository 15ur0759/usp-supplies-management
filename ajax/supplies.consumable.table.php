    ...
    <tr>
        ...
        <td class="text-center"><div class="form-check form-switch">
            ...
            <div class="table-controls text-center d-flex justify-content-center align-items-center">
                ...
                <div onclick="editConsumable('<?=$consumable['item_id']?>','<?=$consumable['item_code']?>','<?=$consumable['item_description']?>','<?=$consumable['item_category']?>','<?=$consumable['measurement']?>','<?=$consumable['quantity']?>','<?=$consumable['supplier_id']?>')" data-toggle="modal" data-target="#editConsumable" class="table-control table-control-edit">
...
<script>
     $(document).ready( function () {
        $('#consumableTable').DataTable({order:[[0,'desc']]});
    } );
